...
//its neighbors. More precisely, it is an index
//P such that 0 < P < N − 1,  A[P − 1] < A[P]
//and A[P] > A[P + 1].

def solution(A: Array[Int]): Int = {
  val N = A.length
  val peaks = Array[Integer]()
  for (i <- 1 until N - 1 if A(i) > A(i - 1) && A(i) > A(i + 1)) peaks :+ i
  var size = 1
  while (size <= N) {
    if (N % size == 0){
      var find = 0
      ...
      var ok = true
      for (peakIdx <- peaks) {
        if (peakIdx / size > find)  ok = false
        else if (peakIdx / size == find) find += 1
      }
...
//
//We want to divide this array into blocks containing
//the same number of elements. More precisely, we want
//to choose a number K that will yield the
//following blocks:
//
//A[0], A[1], ..., A[K − 1],
//A[K], A[K + 1], ..., A[2K − 1],
...
//be peaks, but only if they have both neighbors
...
//  because the (1, 2, 3) blocks do not contain a peak. Notice
//  in particular that the (4, 3, 4) block contains two peaks: A[3] and A[5].
//
